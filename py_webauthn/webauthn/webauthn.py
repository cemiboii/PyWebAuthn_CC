...
# Only supporting 'Anonymization CA', 'None', 'Basic', and 'Self Attestation' attestation types for now.
AT_BASIC = 'Basic'
...
SUPPORTED_ATTESTATION_TYPES = (AT_BASIC, AT_NONE, AT_SELF_ATTESTATION, AT_ANONYMOUS_CA)
...
# Only supporting 'apple, 'fido-u2f', 'packed' and 'none' attestation formats for now.
SUPPORTED_ATTESTATION_FORMATS = (AT_FMT_APPLE, AT_FMT_FIDO_U2F, AT_FMT_PACKED, AT_FMT_NONE)
...
class WebAuthnMakeCredentialOptions(object):
    ...
    def __init__(self, challenge, rp_name, rp_id, user_id, username,
                 display_name, registration_date, timeout=60000, attestation='direct',
    ...
    def registration_dict(self):
        ...
        if self.registration_date:
            registration_dict['user']['regg'] = self.registration_date
...
class WebAuthnUser(object):
    def __init__(self, user_id, username, display_name, registration_date,
...
class WebAuthnRegistrationResponse(object):
    ...
    def verify(self):
        try:
            ...
            # in authData.
            trust_anchors = _get_trust_anchors(attestation_type, fmt, self.trust_anchor_dir, self.additional_trust_anchors)
